...
f.addEventListener('submit', async e => {
  ...
  try {
    ...
    def _submit_job(self):
        ...
        #  - APPEND: add_to_pres_uuid + files (name and library_adds ignored)
        #  - NEW:    name + files
        #  - LIBRARY ADDS: library_adds (with optional playlist_uuid)
        has_append = bool(add_to_pres_uuid and files)
        has_new = bool(name and files) and not has_append
        has_existing = bool(library_adds) and not has_append
        if not has_append and not has_new and not has_existing:
            return self._send_json(400, {"error": "need either (name + files), library_adds, or (add_to_pres_uuid + files)"})
        jid = str(uuid.uuid4())
        ...
        if has_append:
            display_name = f"+{len(saved)} slide(s) appended"
        else:
        ...
        with db_lock, db() as c:
            ...
            c.execute("INSERT INTO jobs(job_id, machine_id, name, status, folder, files_json, "
                      ...
                      (jid, machine_id, display_name, "queued", jdir,
                       json.dumps(saved), json.dumps(library_adds), playlist_uuid, add_to_pres_uuid, ts, ts))
            c.commit()
